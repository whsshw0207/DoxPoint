...
// ============ HERO GRAPHIC — FPS sniper overload ============
function HeroGraphic({ accent, intensity = "high" }) {
  // Normalize accent (CSS variable) for SVG stroke — we use `currentColor` on a wrapper.
  const ref = hR(null);
  const [mouse, setMouse] = hS({ x: 0.5, y: 0.5 });
  ...
  // Parallax: reticle follows mouse subtly
  const px = (mouse.x - 0.5) * 24;
  const py = (mouse.y - 0.5) * 24;

  // Bullet trajectory trails — fixed random positions for determinism
  const trails = hM(() => {
    const arr = [];
    for (let i = 0; i < 14; i++) {
  ...
  return (
    <div className="hero-gfx" ref={ref} style={{ color: accent }}>
      ...
      <svg className="gfx-svg" viewBox="0 0 1000 800" preserveAspectRatio="xMidYMid slice"
        ...
        {/* concentric scope rings — pulsing */}
        {[320, 260, 200, 140].map((r, i) => (
          <circle key={i} cx="500" cy="400" r={r}
            fill="none" stroke="currentColor" strokeOpacity={0.18 + i * 0.06}
            strokeWidth="1"
        ...
        {/* RETICLE — crosshair core (mouse parallax via parent transform) */}
        <g className="reticle" style={{ transformOrigin: "500px 400px" }}>
          ...
          <g style={{ animation: "retSpin 20s linear infinite", transformOrigin: "500px 400px" }}>
            <polygon points="500,320 580,400 500,480 420,400"
          ...
          <circle cx="500" cy="400" r="10" fill="none" stroke="currentColor" strokeWidth="1" strokeOpacity="0.6"
        ...
        {/* Distance / angle labels around reticle */}
      ...
      {/* ===== CENTER TYPE — "DOX.POINT" callsign ===== */}
      <div className="gfx-center" style={{ transform: `translate(calc(-50% + ${px * 0.15}px), calc(-50% + ${py * 0.15}px))` }}>
        ...
        <div className="gfx-center-bot mono">
          <span>[</span><span>COACH · DOX</span><span>]</span>
          <span className="sep">/</span>
          <span>[</span><span>CAL .338</span><span>]</span>
          <span className="sep">/</span>
          <span>[</span><span>8Y PRO</span><span>]</span>
        </div>
      </div>
      ...
      {floaters.map((f, i) => (
        <span key={i}
          ...
          style={{
            left: `${f.x}%`, top: `${f.y}%`,
            fontSize: f.s, transform: `rotate(${f.rot}deg)`,
      ...
      <div className="gfx-hud gfx-hud-br mono">
        <div className="hud-mini">
          <div className="hud-bar"><div className="hud-bar-fill" style={{ width: "92%" }} /></div>
          <div className="hud-row"><span className="k">ACCURACY</span><span className="v">92%</span></div>
        </div>
        <div className="hud-mini">
          <div className="hud-bar"><div className="hud-bar-fill" style={{ width: "88%" }} /></div>
          <div className="hud-row"><span className="k">POSITION</span><span className="v">88%</span></div>
        </div>
        <div className="hud-mini">
          <div className="hud-bar"><div className="hud-bar-fill" style={{ width: "95%" }} /></div>
          <div className="hud-row"><span className="k">GAME SENSE</span><span className="v">95%</span></div>
        </div>
      </div>
...
Object.assign(window, { HeroGraphic });
